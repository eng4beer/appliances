...
echo "######################"
virt-customize -a $image --run-command "subscription-manager register --username=$name --password=$password" --run-command "subscription-manager attach --pool $pool_id" --run-command 'subscription-manager repos --disable "*"' --run-command 'subscription-manager repos --enable rhel-7-server-rpms --enable rhel-7-server-extras-rpms --enable rhel-7-server-ansible-2.6-rpms'
echo "# Pulling packages #"
echo "####################"
virt-customize -a $image --run-command 'yum -y install ansible' --run-command "curl -o /opt/ansible-tower-setup-bundle-latest.el7.tar.gz $tower_url"
